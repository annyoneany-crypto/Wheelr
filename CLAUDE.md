...
- **Long-form SEO copy is data, not templates** (`wheel-templates.seo.ts`), so decide deliberately whether a string belongs in XLIFF or in a locale-keyed object before adding it. Note the build scripts parse that file with regexes, so wrapping its fields in `$localize` means updating `tools/generate-template-md.mjs` in the same commit.

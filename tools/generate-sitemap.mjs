...
/**
 * Locale subpaths, mirroring `angular.json` and `src/app/services/i18n.config.ts`.
 * The source locale lives at the root, so its prefix is empty.
 */
const LOCALE_SUBPATHS = ['', 'it', 'de', 'fr', 'es', 'zh'];

function localized(subPath, loc) {
  const prefix = subPath ? `/${subPath}` : '';
  return loc === '/' ? `${prefix}/` : `${prefix}${loc}`;
}
...
// pad the sitemap for nothing.
const body = allPages
  .flatMap((page) => {
    const lastmod = lastModified(page.sources);

    return LOCALE_SUBPATHS.map((subPath) =>
      [
        '  <url>',
        `    <loc>${ORIGIN}${localized(subPath, page.loc)}</loc>`,
        `    <lastmod>${lastmod}</lastmod>`,
        `    <changefreq>${page.changefreq}</changefreq>`,
        `    <priority>${page.priority}</priority>`,
        ...(subPath ? [] : (page.images ?? []).map(imageBlock)),
        '  </url>',
      ].join('\n')
    );
  })
...
console.log(
  `sitemap.xml: ${allPages.length * LOCALE_SUBPATHS.length} urls ` +
    `(${allPages.length} pages × ${LOCALE_SUBPATHS.length} locales, ${templatePages.length} of them template landing pages)`
);
